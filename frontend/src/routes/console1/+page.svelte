<script>
  ...
  // Analytics Panel import removed
  ...
  // Column widths as percentages - adjusted for two-panel layout
  let conversationWidth = 30; // ~30%
  let assistantWidth = 70;    // ~70%
  ...
  function handleResize(event) {
    ...
    if (activeHandle === 'conversation-assistant') {
      ...
      const newAssistantWidth = 100 - newConversationWidth;
      
      conversationWidth = newConversationWidth;
      assistantWidth = newAssistantWidth;
    }
  }
...
<div class={`h-screen flex flex-col bg-gray-50 dark:bg-gray-900 ${$darkMode ? 'dark' : ''}`}>
  ...
  <div class="flex-1 p-6 gap-0 flex min-h-0" bind:this={containerEl}>
    <!-- Left Panel - Conversation or Meeting -->
    <div class="h-full flex flex-col min-h-0" style="width: {conversationWidth}%;">
    ...
    <!-- AI Assistant Panel -->
    <div class="h-full flex flex-col min-h-0" style="width: {assistantWidth}%;">
      <AIAssistantPanel 
